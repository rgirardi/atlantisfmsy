...
#'
#' @description Create the folder \code{folder_name} in the working directory
#'   \code{folder_path}. It copies/pastes inside it the files necessary to run
#'   Atlantis from the \code{model_path} directory.
...
#'   to run your model. If not provided, the function will search for the unique
#'   batch file in your \code{folder_path}. \strong{Default:} NULL.
#' @param folder_name The name of the simulation folder to contain a set of F scenarios.
#' @return \code{simu_path} The simulation directory for the F scenarios considered.
#' @examples
...
atlantisf_modelcopy = function(folder_path, model_path, exe_name, batch_file = NULL, folder_name) {
  #check os used
  os <- Sys.info()['sysname']
  ...
  setwd(model_path)
  if (os == "Windows") {
    copy <- paste("xcopy /e /y ", shortPathName(model_path), " ", paste(shortPathName(folder_path), folder_name, sep = "\\"), sep = "") #copy model information.
    shell(copy)
  } else {
    copy <- paste("cp -a ", model_path, "/. ", paste(folder_path, folder_name, sep = "/"), "/", sep = "") #copy model information.
    system(copy)
    convert <- paste("cd ", paste(folder_path, folder_name, sep = "/"), "/", "\n", "flip -uv *", sep = "") #convert Windows text files into Linux text files.
    system(convert)
  }

  simu_path <- file.path(folder_path, folder_name)
  gc()
  ...
  if(any(substring(line_para, 1, 2) == "..")){
    ...
    #Check if forcing folder already exist in the parent directory of the simulation path. ADD A SCRIPT THAT CHECK THAT ALL THE FILES ARE PRESENT IN /AtlantisMSY/forcing_folder IN CASE OF COMPUTER CRASH DURING THE COPY OF FILES.
    if (!dir.exists(file.path(folder_path, forcing_folder))) {
      dir.create(file.path(folder_path, forcing_folder), showWarnings = FALSE) #create forcing folder.
      forcing_path <- gsub(utils::tail(strsplit(model_path, "/")[[1]], 1), forcing_folder, model_path)

      # copy content of the forcing folder into the new forcing folder used for simulation.
      if (os == "Windows") {
        copy <- paste("xcopy /e /y ", shortPathName(forcing_path), " ", paste(shortPathName(folder_path), forcing_folder, sep = "\\"), sep = "") #copy model information.
        shell(copy)
      } else {
        copy <- paste("cp -a ", forcing_path, "/. ", paste(folder_path, forcing_folder, sep = "/"),"/", sep = "") #copy model information.
        system(copy)
        convert <- paste("cd ",paste(folder_path, forcing_folder, sep = "/"), "/", "\n", "flip -uv *", sep = "") #convert windows text files into Linux text files.
        system(convert)
      }
    }
  }
