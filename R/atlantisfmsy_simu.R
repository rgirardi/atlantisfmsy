...
atlantisfmsy_simu = function(func_grp, folder_path, model_path, exe_name, burnin_time, fmax, batch_file = NULL, fmin = NULL, restart = 0) {
  ...
  if(restart == 0) {
    ...
    atlantisfmsy_fmaxcatch(func_grp, simu_path, exe_name, run_time, fmax, batch_file) #determine the maximum yield and the F associate. Then determine the next set of F to simulate. return end = 1 if Fmsy is reached or 2 if maximum yield is reached for fmax.
  ...
  while(end == 0) {
    ...
    atlantisfmsy_fmaxcatch(func_grp, simu_path, exe_name, run_time, fmax, batch_file)  #determine the maximum yield and the F associated.
